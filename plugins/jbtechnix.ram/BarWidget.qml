...
BarWidget {
  ...
  function parseStats(output) {
    ...
    root.available = formatBytes(Number(fields[2]))
  ...
  Process {
    id: statsProcess
    command: ["bash", "-c", "awk '/^MemTotal:/ {total=$2*1024} /^MemAvailable:/ {available=$2*1024} /^SwapTotal:/ {swapTotal=$2*1024} /^SwapFree:/ {swapFree=$2*1024} END {used=total-available; swapUsed=swapTotal-swapFree; printf \"%d %d %d %.0f%% %d %d\\n\", used, total, available, used*100/total, swapUsed, swapTotal}' /proc/meminfo"]
  ...
  WidgetButton {
    ...
    anchors.fill: parent
    bar: root.bar
    text: "󰍛 " + root.used + "/" + root.total
    labelVisible: true
    tooltipText: "RAM " + root.percent + " used · click for details"
    horizontalMargin: 8
    verticalPadding: 8
  }
